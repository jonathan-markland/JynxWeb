...
namespace JynxFramework
{
	...
	String::String(const Slice<const char>& slice)
	{
		ConstructFromPart(slice.Start(), slice.Count());
	}
	...
	{
		ConstructFromPart(s, StringLengthOf(s));
	}

	void String::ConstructFromPart(const char* s, uintptr_t length)
	{
...
namespace JynxFramework
{
	...
	StringBuilder& StringBuilder::Append(const Slice<const char>& slice)
	{
		return Append(slice.Start(), slice.Count());
	}
